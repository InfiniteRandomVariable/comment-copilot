...
| 2 | PENDING | `pnpm --filter @copilot/web test:inbox`; `pnpm verify:phase-boundary`; `pnpm ci:check` | `apps/web/app/inbox/page.tsx`, `apps/web/app/inbox/actions.ts`, `apps/web/app/inbox/filtering.ts`, `apps/web/tests/inbox.actions.integration.test.ts` | Pending | Added queue filters (platform/intent/search), queue summary cards, and filter-preserving action/pagination redirects. |
