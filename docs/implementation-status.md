...
- Inbox review queue now supports high-volume operator filtering (platform, intent, text search), queue split summaries with stale-age signals, per-item age labels, quick approve+send action, and preserves filter state across approve/send/reject actions and pagination.
